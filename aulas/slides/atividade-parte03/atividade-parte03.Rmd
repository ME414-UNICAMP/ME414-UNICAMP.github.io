---
title: ME414 - Estatística para Experimentalistas
subtitle: Qual a idade?
output:
  ioslides_presentation:
    widescreen: yes
logo: ../logo-imecc.png
---

# Qual a idade?

## Coleta de dados

Cada aluno pega uma folha e recorta 10 tiras de papel. 

Cada tira deverá estar indicada por um número (que representa a Pessoa na foto que será apresentada).

Ao lado da indicação do número da Pessoa, coloque um "chute" para a idade da pessoa.

Não consulte seus colegas, coloque o seu "chute" sobre a idade.




## Pessoa 1

```{r,echo=FALSE}
# http://www.aceshowbiz.com/events/Daryl%20Hannah/daryl-hannah-stars-2012-benefit-gala-01.html

# foto de 2012

idade=52
```
<center><img src="figuras/daryl-hannah-stars-2012-benefit-gala-01.jpg" width=450></center>


## Pessoa 2

```{r,echo=FALSE}
# https://pt.wikipedia.org/wiki/Jodie_Foster
# foto de 1989

idade=27
```
<center><img src="figuras/Jodie_Foster_at_the_1989_Academy_Awards.jpg" width=450></center>


## Pessoa 3

```{r,echo=FALSE}
# http://static1.purepeople.com/articles/1/17/35/21/@/2092987-johnny-depp-a-cannes-en-1992-950x0-2.jpg
# foto de 1992

idade=29
```
<center><img src="figuras/2092987-johnny-depp-a-cannes-en-1992-950x0-2.jpg" width=450></center>


## Pessoa 4


```{r,echo=FALSE}
# http://www.gettyimages.com/event/liam-gallagher-self-assignment-june-19-2017-700070321?esource=SEO_GIS_CDN_Redirect#singer-and-musician-liam-gallagher-is-photographed-on-june-19-2017-in-picture-id800194674
# foto de 2017

idade=45
```
<center><img src="figuras/Screen Shot 2017-08-06 at 13.41.06.png" width=550></center>



## Pessoa 5

<center><img src="figuras/Elton John1968.jpg" width=500></center>

```{r,echo=FALSE}
# Elton John
# foto de 1968
idade=21
```

## Pessoa 6


<center><img src="figuras/A-66089-1336554906-5167.jpg" width=500></center>

```{r,echo=FALSE}
# Bonnie Prince Billy
idade=33
```

## Pessoa 7


<center><img src="figuras/400px-Christie_Brinkley_at_Met_Opera_4.jpg" width=600></center>

```{r,echo=FALSE}
idade=54
```

## Pessoa 8


<center><img src="figuras/zach-galifianakis-wallpaper-59415-61201-hd-wallpapers.jpg" width=600></center>

```{r,echo=FALSE}
idade=47
```

## Pessoa 9

<center><img src="figuras/13465976_10154674692372923_8617974188435350826_n.jpg" width=500></center>

```{r,echo=FALSE}
idade=35
```

## Pessoa 10

<center><img src="figuras/marcelo_knobel_reitoriaunicamp03-8051.jpg" width=600></center>

```{r,echo=FALSE}
idade=48
```


## Grupos {.smaller}

Formam-se 10 grupos.

Grupo X fica responsável por coletar os dados (tiras de papel referentes à Pessoa X) dos chutes referentes à Pessoa X. 

Cada grupo deve entregar uma folha com:

* Título: Pessoa X
* RA e nome de cada membro do grupo X
* estatísticas sumárias (mínimo, máximo, média)
* histograma 
* ramo-e-folhas
* uma estimativa para a idade da Pessoa X, baseando-se na análise descritiva realizada

A presença será contabilizada pela entrega dessa atividade ao final da aula.


##

Slides produzidos por:

* Samara Kiihl
