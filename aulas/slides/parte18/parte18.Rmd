...
* $H_A: p \neq p_0$  (bilateral): p-valor=$P(|Z| ≥|z_{obs}|)$ <br />
* $H_A: p < p_0$ (unilateral à esquerda): p-valor=$P(Z \leq z_{obs})$ <br />
...
* Se p-valor $\leq \alpha$: rejeitamos $H_0$ <br />
* Se p-valor > $\alpha$: não rejeitamos $H_0$


<!-- ## p-valor Teste Bilateral -->

<!-- $H_A: p \neq p_0$  (bilateral) $\quad \Rightarrow \quad$ p-valor=$P(|Z| ≥|z_{obs}|)$ -->

<!-- <center><img src="figuras/pvalor_bilateral.png" width=900 height=350></center> -->


<!-- ## p-valor Teste Unilateral -->

<!-- $H_A: p > p_0$ (unilateral à direita) $\quad \Rightarrow \quad$ p-valor=$P(Z \geq z_{obs})$ -->
...
Os alunos dizem que conseguem distinguir entre Coca-Cola normal e Coca-Cola Zero.

Um experimento foi feito em sala de aula para comprovar se a afirmação é verdadeira.

Um aluno experimentou, em ordem aleatória, 20 amostras (ao acaso era Coca normal ou zero) e anotou-se a quantidade de acertos.
...
Dos 20 testes, o aluno acertou `r acertos`! Temos então uma proporção amostral de acertos $\hat p = `r acertos`/`r n`=`r phat`$.  
Isso mostra que o aluno realmente sabe a diferença?
...
<!-- > **p-valor** = $P(Z \geq `r zobs`) = `r format(valorpn, digits=1,scientific=FALSE)`$ -->
...
<!--        col = COL[1], -->
<!--        lwd = 1.5) -->
<!-- text(3.5, yMax / 2, labels= paste0("p-valor = ", format(valorpn,digits=1,scientific=FALSE)), -->
<!--      pos = 3, -->
<!--      cex = 1.2, -->
<!--      col = COL[1]) -->
<!-- ``` -->
...
**Hipóteses:**  $H_0: \text{Acertos} = 10	\quad \mbox{vs} \quad H_A: \text{Acertos} > 10$
...
**p-valor** = $P(T\geq `r acertos`)=`r format(valorp, digits=1,scientific=FALSE)`$
...
<br /><br /><br /><br />
$H_0: \text{você não está grávida(o)}$

$H_A: \text{você está grávida(o)}$
...
Então, optamos por controlar $\alpha=$P(Erro Tipo I), que é considerado o erro mais grave. Geralmente fixamos $\alpha=0.05$ e rejeitamos $H_0$ se p-valor $<\alpha$.
...
O teste é bilateral, portanto o p-valor é calculado como:

**p-valor**: $P(|Z| \geq 7.5) = 2P(Z \geq 7.5) \approx 0$
...
**Hipóteses** $\quad H_0: \mu=\mu_0$ vs $H_A: \mu \neq \mu_0$  
Para um **nível de significância** $\alpha$, definimos a **região crítica** do teste:

```{r, echo=FALSE, fig.align = 'center', fig.height=4.2, fig.width=7, results='hide', message=FALSE}
library(openintro)
source("../functions/BuildNullX.R")

par(mar = c(4, 4, 1, 2))
BuildNullX()
segments(1.96 * c(-1, 1), rep(0, 2), y1 = rep(0.15, 2),
...
```{r, echo=FALSE, fig.align = 'center', fig.height=4.2, fig.width=7, results='hide'}
source("../functions/BuildNullXuni.R")

par(mar = c(4, 4, 1, 2))
BuildNullXuni()
segments(1.65 , rep(0, 2), y1 = rep(0.15, 2),
         col = COL[4], lty = 3, lwd = 3)
segments(1.65 , rep(0, 2), y1 = rep(0.15, 2),
         col = COL[4], lty = 3, lwd = 1.5)
...
**p-valor:** $P(|t_{n−1}| ≥ |t_{obs}|) = 2P(t_{n−1} ≥ |t_{obs}|)$  
...
**p-valor:** <font size=5>$P(|t_{24}| ≥ 10.56) = 2P(t_{24} ≥ 10.56) \approx 0$ </font>
...
$\qquad \mbox{p-valor}=P(Z<-18.3) \approx 0$
