...
<center>
![Pesquisa](figuras/graficofake2.png "Pesquisa")
</center>
...
<font size=4> $$\frac{45}{224} = `r round(45/224, 2)` = `r round(45/224, 2)*100`\%$$ </font>

Proporção de pacientes do grupo controle que sofreram infarto: 
<font size=4> $$\frac{28}{227} = `r round(28/227, 2)` = `r round(28/227, 2)*100`\%$$
</font>
...
<center><img src="figuras/sins1.png" width=200></center>
...
dados <- dados %>% select(spam, num_char, line_breaks, format, number) %>% 
    mutate(num_char = num_char*1000)
...
ggplot(whos, aes(doctor, prop)) +
    geom_bar(stat = "identity") +
    coord_flip() + ylab("Proporção") + xlab("Atores") +
    ggtitle("Gráfico de Barras - Proporções") +
    theme(axis.title = element_text(face="bold", size=14, color="black"),
          axis.text = element_text(face="bold", size=12, color="black"),
          plot.title = element_text(size=18)) 
```
...
```{r, echo=FALSE}
dias <- c(2,2,0,0,5,8,3,4,1,0,0,7,1,7,1,5,4,0,4,0,1,8,9,7,0,
...
<font size=5>
```{r, echo=FALSE}
licenca <- as.data.frame(table(dias))
licenca$prop <- round(licenca$Freq/length(dias), 2)

licenca1 <- tbl_df(licenca)
colnames(licenca1) <- c("Dias de Licença","Frequência","Proporção")

kable(licenca1, format="html", align = "c")
```
</font> 
...
ggplot(licenca, aes(dias, Freq)) +
    geom_bar(stat = "identity", fill="blue") + 
    ylab("Frequência") + xlab("Dias de Licença") +
    ggtitle("Gráfico de Barras dos Dias de Licença - Frequência") + 
...
```{r, echo=FALSE}
ggplot(licenca, aes(x=dias, y=prop)) +
    geom_bar(stat = "identity", fill="blue") + 
    ylab("Proporção") + xlab("Dias de Licença") +
    ggtitle("Gráfico de Barras dos Dias de Licença - Proporção") + 
    theme(axis.text = element_text(face="bold", size=12, color="black"),
          axis.title = element_text(face="bold", size=12, color="black"),
          plot.title = element_text(size=18))
```
