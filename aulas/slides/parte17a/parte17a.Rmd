...
> * 10 xícaras: 5 com chá colocado antes do leite e 5 com leite antes do chá.

> * As dez xícaras foram apresentadas em ordem aleatória para a senhora, mas a informação de que eram 5 de cada tipo foi passada a ela.

> * A senhora deveria provar a bebida das oito xícaras e escolher as 5 xícaras que acreditava estar com chá primeiro.

> * Verificou-se quantas dentre as 5 xícaras ela escolheu corretamente.
...
* Tarefa da senhora: escolher as 5 xícaras com chá primeiro.

> * São 10 xícaras: 5 com leite primeiro e 4 com chá primeiro.

> * A senhora escolhe 5 dentre 10, sem reposição. 
...
* É possível que ela escolha as 5 corretamente: 5 com chá primeiro.

> * De quantas formas ela pode escolher 5 corretas?

> * Para escolher 5 corretas, duas coisas devem ocorrer: escolher 5 com chá primeiro (dentre 5 com chá primeiro) e não escolher nenhuma dentre as 5 com leite primeiro. 

> * De quantas formas é possível fazer isso, caso ela de fato não saiba distinguir e esteja fazendo tudo ao acaso?

$$\binom{5}{5}\binom{5}{0}=`r choose(5,5)*choose(5,0)`$$

## Resultados possíveis {.build}

* É possível que ela escolha 4 corretamente: 4 com chá primeiro e 1 com leite primeiro.

> * De quantas formas ela pode escolher 4 corretas, caso ela de fato não saiba distinguir e esteja fazendo tudo ao acaso?

$$\binom{5}{4}\binom{5}{1}=`r choose(5,4)*choose(5,1)`$$


## Resultados possíveis {.build}

* É possível que ela escolha 3 corretamente: 3 com chá primeiro e 2 com leite primeiro.

> * De quantas formas ela pode escolher 3 corretas, caso ela de fato não saiba distinguir e esteja fazendo tudo ao acaso?

$$\binom{5}{3}\binom{5}{2}=`r choose(5,3)*choose(5,2)`$$

## Resultados possíveis {.build}

* É possível que ela escolha 2 corretamente: 2 com chá primeiro e 3 com leite primeiro.

> * De quantas formas ela pode escolher 2 corretas, caso ela de fato não saiba distinguir e esteja fazendo tudo ao acaso?

$$\binom{5}{2}\binom{5}{3}=`r choose(5,2)*choose(5,3)`$$


## Resultados possíveis {.build}

* É possível que ela escolha 1 corretamente: 1 com chá primeiro e 4 com leite primeiro.

> * De quantas formas ela pode escolher 1 correta, caso ela de fato não saiba distinguir e esteja fazendo tudo ao acaso?

$$\binom{5}{1}\binom{5}{4}=`r choose(5,1)*choose(5,4)`$$


## Resultados possíveis {.build}

* É possível que ela erre todas: 0 com chá primeiro e 5 com leite primeiro.

> * De quantas formas ela pode errar todas, caso ela de fato não saiba distinguir e esteja fazendo tudo ao acaso?

$$\binom{5}{0}\binom{5}{5}=`r choose(5,0)*choose(5,5)`$$

## Resultados possíveis {.build}


```{r,echo=FALSE,fig.align='center'}
barplot(c(choose(5,5)*choose(5,0),choose(5,4)*choose(5,1),choose(5,3)*choose(5,2),choose(5,2)*choose(5,3),choose(5,1)*choose(5,4),choose(5,0)*choose(5,5)), names.arg=0:5, col="blue", main="", 
...
* Extrações casuais sem reposição
* $N$ objetos: 10 xícaras
* $r$ têm a característica A: 5 com chá primeiro
* $N-r$ têm a característica B: 5 com leite primeiro   
* Um grupo de $n$ elementos é **escolhido ao acaso**, dentre os $N$ possíveis, sem reposição: a senhora escolhe 5 xícaras dentre as 10.

## Teste de hipótese

Queremos calcular a probabilidade de que dentre as 5 xícaras escolhidas $x$ tenham de fato o chá colocado primeiro. 

Seja $X$ a v.a. que representa o número de xícaras com chá primeiro dentre as 5 selecionadas.

Então dizemos que $X$ segue uma distribuição **Hipergeométrica** com parâmetros $N,n,r$, ou seja, $X \sim Hip(N=10,n=5,r=5)$.

A probabilidade de se observar $x$ é dada por: 
$$P(X=x)=\frac{\binom{5}{x}\binom{5}{n-x}}{\binom{10}{5}}, \qquad 0\leq x \leq 5$$ 
...
$$P(X=x)=\frac{\binom{5}{x}\binom{5}{5-x}}{\binom{10}{5}}, \qquad 0\leq x \leq 5$$
...
```{r,echo=FALSE,fig.align='center'}
barplot(c(choose(5,5)*choose(5,0),choose(5,4)*choose(5,1),choose(5,3)*choose(5,2),choose(5,2)*choose(5,3),choose(5,1)*choose(5,4),choose(5,0)*choose(5,5))/choose(10,5), names.arg=0:5, col="blue", main="", 
...
> Como decidir se rejeitamos a hipótese de que a senhora não consegue distinguir os chás, sendo que ela acertou, por exemplo, 4? Se ela tivesse acertado todas as 5 xícaras? Seria por pura sorte? Ou ela tem algum conhecimento?
...
Se a senhora acertou 4 xícaras:

```{r,echo=FALSE,warning=FALSE}
library(MASS)
p0 = fractions(dhyper(0,5,5,5))
p1 = fractions(dhyper(1,5,5,5))
p2 = fractions(dhyper(2,5,5,5))
p3 = fractions(dhyper(3,5,5,5))
p4 = fractions(dhyper(4,5,5,5))
p5 = fractions(dhyper(5,5,5,5))
pvalue = fractions(dhyper(4,5,5,5)+dhyper(5,5,5,5))
```

$$P(X=4) = \frac{\binom{5}{4}\binom{5}{1}}{\binom{10}{5}}=`r as.character(p4)` $$
...
> Se a senhora tivesse acertado 5, seria ainda mais evidência contra $H_0$, de forma que o valor de p é calculado como:

$$P(X=4)+P(X=5)=`r as.character(p4)` + `r as.character(p5)` = `r as.character(pvalue)`$$
