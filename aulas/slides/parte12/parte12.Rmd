...
> A probabilidade de que uma v.a. $X$ contínua pertença ao intervalo da reta ($-\infty,x$] daremos o nome de **função de distribuição acumulada** (f.d.a.), e a denotaremos por $F_{X}$
...
$$Var(X) = \int_{-\infty}^{\infty}[x - \mathbb E(X)]^{2}f_{X}(x)dx = \int_{-\infty}^{\infty}(x-1)^2f_{X}(x)dx = \frac{1}{6}$$
...
> $\mathbb E[X]=  800$. Como $X$ tem distribuição exponencial com parâmetro $\lambda$, $E[X]= \frac{1}{\lambda}=800$, portanto $\lambda= \frac{1}{800}$.  
