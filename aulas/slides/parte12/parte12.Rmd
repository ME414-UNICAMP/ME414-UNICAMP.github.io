...
1. $f_{X}(x)\geq0$ para todo $x \in \mathbb{R}$.  
...
Item 1 -  Devemos escolher $C$ de modo que $f(x)$ satisfaça:
   ...
   + $f_{X}(x)\geq0$ para todo $x \in \mathbb{R}$.
...
$$
\begin{aligned}
\mathbb E(X) &= \int_{-\infty}^{\infty} \!\! x f_{X}(x) dx =  \int_0^{1/2} \! 4x^2dx +  \int_{1/2}^{1} \! 4x(1-x)dx\\ 
...
A função de distribuição acumulada de uma variável aleatória contínua é dada por $$F_{X}(x) = \int_{-\infty}^x f(t) dt$$
