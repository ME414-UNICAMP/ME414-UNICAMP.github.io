...
trat <- c("A","A","A","A","A","B","B","B","B","B")

id <- 1:n

set.seed(22020)
resposta <- c(rpois(n1,lambda=14),rpois(n2,lambda=20))
#[1] 10 16 13 18 16 33 24 19 29 29
...
Desta maneira, uma valor de diferença como o observado ou ainda mais extremo pode ocorrer ao acaso com probabilidade `r mean(abs(diff)>=abs(xbarA-xbarB))`. Se considerarmos um nível de significância $\alpha=0.05$, concluímos que dados portanto trazem evidências para rejeitar a hipótese de que não há diferença entre os tratamentos.
...
set.seed(22020)
resposta <- c(rpois(n1,lambda=14),rpois(n2,lambda=20))
#  [1] 10 16 13 18 16 25 17 13 22 21 18 32 17 21 16 13 24 34 16 24
