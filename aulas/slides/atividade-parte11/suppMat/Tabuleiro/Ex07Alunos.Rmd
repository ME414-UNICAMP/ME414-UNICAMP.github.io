...
```{r,echo=FALSE}
PontoAmostral=c(XXXXXXXXXXXXXXXX)
...
```{r,echo=FALSE}
ExperimentoAleatorioVitoria <- ExperimentoAleatorio[ExperimentoAleatorio$PontoAmostral %in% c(XXXXXXXXXXX),]
...
O resultado de uma partida deste jogo pode ser considerado como uma variável aleatória, $X$, tendo os seguintes resultados possíveis ganhar (1) ou perder (0).  A probabilidade de vencer o jogo é $P(X=1)=P(A)=`r XXXXXXXX`$ e a de perder é $P(X=0)=P(B)=`r XXXXXXXX`$. 
...
$$E(X)=1\times P(X=1) + 0\times P(X=0)=`r XXXXXXX`\,.$$
...
$E(X^2)=1 \times P(X=1) + 0\times P(X=0)=`r XXXXXXX`$.

Portanto $Var(X)=E(X^2)-[E(X)]^2=XXXXXXX$.
...
Sabemos que $P(X_i=1)=`r XXXXXXX`$.

Seja $Y$ o número de partidas ganhas de um total de $n$ partidas. Se as partidas forem independentes, então $Y=\sum_{i=1}^nX_i\sim\mbox{Binomial}(n,p=`r XXXXXXX`)$.
...
----------------------------------- | ------------- | ------------
$X_1=0$, $X_2=0$, $X_3=0$           | `r XXXXXX` | 0
$X_1=1$, $X_2=0$, $X_3=0$         | `r XXXXXXX`| 1
$X_1=0$, $X_2=1$, $X_3=0$         | `r XXXXXXX`| 1
$X_1=0$, $X_2=0$, $X_3=1$         |`r XXXXXXX`| 1
$X_1=1$, $X_2=1$, $X_3=0$         |`r XXXXXXX`| 2
$X_1=1$, $X_2=0$, $X_3=1$         |`r XXXXXXX`| 2
$X_1=0$, $X_2=1$, $X_3=1$         |`r XXXXXXX`| 2
$X_1=1$, $X_2=1$, $X_3=1$         |  `r XXXXXXX` | 3
...
* a probabilidade de vencer 4 vezes dentre 10 partidas é: `r dbinom(x=XXX,size=XXXX,prob=XXXXX)`.
* a probabilidade de vencer 5 vezes dentre 7 partidas é: `r dbinom(x=XXX,size=XXX,prob=XXXXX)`.
* a probabilidade de vencer 1 dentre 3 partidas é: `r dbinom(x=XXX,size=XXX,prob=XXX)`.

Para calcular $P(Y\leq y)$ ou $P(Y>y)$ automaticamente, usamos a função **pbinom**. Usando esta função, preencha a seguir:

* a probabilidade de vencer no máximo 2 vezes dentre 3 partidas é `r pbinom(q=XXX,size=XXXX,prob=XXXXX,lower.tail=XXXXX)`.

* a probabilidade de vencer pelo menos 7 vezes dentre 10 partidas é `r pbinom(q=XXXX,size=XXX,prob=XXXX,lower.tail=XXXX)`.

* a probabilidade de vencer pelo menos 5 vezes dentre 7 partidas é `r pbinom(q=XXXX,size=XX,prob=XXXXX,lower.tail=XXXX)`.
...
* probabilidade da primeira vitória ocorrer na décima partida: `r dgeom(x=XXX,prob=XXXX)`.

* probabilidade de perder 5 vezes até que a primeira vitória aconteça: `r dgeom(x=XXX,prob=XXXXX)`.

* probabilidade da primeira vitória acontecer na segunda partida: `r dgeom(x=XXXX,prob=XXXX)`.

* a probabilidade de perder 2 vezes até que a primeira vitória aconteça: `r dgeom(x=XXX,prob=XXXXX)`.
