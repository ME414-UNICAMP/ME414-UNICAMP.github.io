...
> * a distribuição amostral de $\bar X_{n}$ aproxima-se de uma **distribuição Normal** de média $\mu$ e variância $\frac{\sigma^{2}}{n}$, quando $n$ for suficientemente grande:
...
Suponha que $X_{i}$ modela o tempo de vida de um transistor em horas. Os tempos de vida de 100 transistores são coletados. Desejamos estudar a variável aleatória $\bar X_{100}$ (média amostral de uma amostra de tamanho 100). 



## Exemplo {.build}

$$\mathbb E(\bar X_{100}) = \mathbb E\left(\frac{1}{n}\sum_{i=1}^{100} X_i \right)=\frac{1}{n}\sum_{i=1}^{100}\mathbb E(X_i) =\frac{1}{2}$$ 

$$ Var(\bar X_{100}) = \frac{1/4}{100} = \frac{1}{400}$$
...
> Se temos uma amostra aleatória simples de tamanho $n$: $X_1,X_2,\ldots, X_n$, pelo TLC sabemos que a distribuição amostral de $\bar X_n$ é aproximadamente Normal$\left( 3.5, \frac{2.92}{n} \right)$.
...
ggplot(df, aes(x = PF)) + 
    geom_histogram(aes(y =..density..),
                   ...
                   colour = "black", 
                   fill = "blue") +  geom_vline(xintercept=mu, color="red") + ylim(0,2.5)+xlim(1,6)+ xlab(" ") + ylab(" ") +ggtitle("Média de 1 dado")
dev.off()
```
...
ggplot(df, aes(x = PF)) + 
    geom_histogram(aes(y =..density..),
                   breaks = seq(1, 6, by = 1/6), 
                   colour = "black", 
                   fill = "blue") +  geom_vline(xintercept=mu, color="red") +ylim(0,2.5)+xlim(1,6)+ xlab(" ") + ylab(" ")  +ggtitle("Média de 2 dados")
dev.off()
```
...
[TCL para proporções](https://gallery.shinyapps.io/CLT_prop/)

[TCL para médias](https://gallery.shinyapps.io/CLT_mean/)
...
<img src="figuras/InferenceGraphicSU17.png" width=350 style="float: right; margin-left: 30px">
...
<sub>[Fonte da imagem](http://www.portalaction.com.br/inferencia-0)</sub>
...
<sub>[Fonte da imagem](https://f.i.uol.com.br/fotografia/2018/08/29/15355904245b874018219ac_1535590424_3x2_md.jpg)</sub>
