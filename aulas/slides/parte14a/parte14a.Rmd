...
* Posso esperar o resultado ads eleições para saber, ou seja, teríamos a resposta de todas as pessoas da cidade.
...
kable(aa,align = 'ccc',col.names = c("Pessoa amostrada 1","Pessoa amostrada 2","$\\hat{p}$"))
```
...
$$\begin{eqnarray} 
E(\hat{p})&=&0\times 0.2 + 0.5\times 0.4 + 1\times 0.4 = 0.6 = p\\
...
Suponha que a resposta de uma pessoa da cidade sobre se vota ou não no candidato $A$ possa ser representada por uma v.a. $X$ que assume o valor $1$ com probabilidade $p$ ou $0$ com probabilidade $1-p$.
