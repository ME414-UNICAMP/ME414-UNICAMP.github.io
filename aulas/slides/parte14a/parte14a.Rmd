...
Tanto pensando que temos uma população "fixa" e retira-se uma amostra aleatória (Modo 1) ou assumindo que cada pessoa da população tenha resposta de acordo com uma variável aleatória e retira-se uma amostra aleatória (Modo 2), temos as mesmas propriedades e comportamento para a proporção amostral $\hat{p}$ a partir de uma amostra de tamanho $n$:
...
Suponha que o salário de uma pessoa possa ser representado por uma **variável aleatória** contínua $X\sim U(1000,5000)$. 

$\begin{aligned}
X &\sim U(1000,5000)\\
\mu=\mathbb E(X) &= \frac{1000+5000}{2}=3000 \\ 
\sigma^2=Var(X) &= \frac{(5000-1000)^2}{12}=1333333
\end{aligned}$
...
> $\begin{aligned}
Var(\bar{X}) &= \frac{1}{n}\sum_{i=1}^n Var (X_i)= \frac{Var(X)}{n}=\frac{\sigma^2}{n}=666666.5\\
\end{aligned}$
...
ggplot(df, aes(x = PF)) + 
    geom_histogram(aes(y =..density..),
                   ...
                   colour = "black", 
                   fill = "blue") +  geom_vline(xintercept=mu, color="red") +
stat_function(fun = dnorm, args = list(mean = mu, sd = sqrt(vari/n)),col="red") + xlab(" ") + ylab(" ") + ylim(0,0.012)+xlim(2000,4000)
```
