...
Imagina que um engenheiro de produção ou um farmacêutico precisa tomar uma decisão importante. Veja alguns cenários: 

* validar um lote de acordo com a proporção de itens defeituosos
* liberar um medicamento de acordo com a proporção de efeios adversos.
 
Essa decisão será feita com base em uma amostra, ou seja, em **dados**. 

Mas será que o que foi observado é suficiente para mudar o que acreditávamos sobre a proporção real?
...
Mas quão pequeno deve ser o p-valor para ser considerado forte evidência contra $H_0$?

Geralmente, fixamos o **nível de significância** do teste ($\alpha$), e usamos a seguinte regra. É comum usarmos $\alpha=0.05$.

 * Se p-valor $\leq \alpha$: rejeitamos $H_0$, ou seja, os dados trazem forte evidência contra a hipótese nula 
 
 * Se p-valor > $\alpha$: não rejeitamos $H_0$, ou seja, não temos evidência nos dados contra a hipótese nula
...
## Passos de um teste de hipótese 
 ...
 Calculamos a probabilidade de um valor igual ou mais extremo ao da estatística do teste observada (p-valor).
...
labels <- expression(1/3,hat(p)[obs])
axis(1, at, labels, cex.axis = 0.8, padj=0.05)
abline(v=1/3,lty=2)
...
arrows(xmax, yMax / 2,
       ...
       col = COL[1],
       lwd = 1.5)
text(xmax, yMax / 2, labels= paste0('p-valor ', valorp),
     pos = 3,
     cex = 1.2,
     col = COL[1])
```
...
**p-valor** (área em azul): probabilidade da proporção amostral assumir um valor igual ao observado, $\widehat p_{obs}$, ou mais extremo, sob $H_0$.
 ...
 * p-valor: $P(Z>z_{obs})=P(Z>0.27)=0.3936$, onde $Z \sim \mbox{N}(0,1)$.
...
* $H_A: p \neq p_0$  (bilateral): p-valor=$P(|Z| ≥|z_{obs}|)$ 
...
## Exemplo: Novo medicamento reduz efeitos adversos {.build}

Um determinado tratamento padrão tinha 20% de efeitos adversos.

Uma indústria farmacêutica desenvolveu um novo medicado e diz que menos de 20% dos pacientes terão efeitos adversos 
...
$$\mbox{p-valor} =P(Z \leq-  1.5) = 0.067$$
...
**Conclusão**: Para $\alpha=0.05$, como o p-valor é maior que 0.05, não temos evidências nos dados para rejeitar a hipótese de que $p=0.20$.
...
$\begin{aligned}
\mbox{p-valor} &=P(|Z| \geq 1.5) =  P(Z \leq -1.5) + P(Z \geq 1.5) \\
...
## Exemplo: Taxa de não conformidade em material hospitalar

Um hospital compra rotineiramente um tipo de material estéril (por exemplo, seringas descartáveis) de um fornecedor que garante que no máximo 2% das unidades chegam não conformes (embalagem violada, selo danificado ou esterilidade comprometida). 

Depois de algumas reclamações internas, a farmácia hospitalar resolveu auditar um lote recente: de $n=400$ unidades inspecionadas, 14 estavam não conformes.

\
**Pergunta:** esse lote está dentro do padrão do fornecedor ou houve uma mudança na taxa de não conformidade?


## Exemplo: Lote de vacina suspeito

Um laboratório informa na bula de uma vacina que sua eficácia é de 95%.

Em uma campanha de vacinação, foi aplicado um lote novo em $n = 150$ pessoas e 136 desenvolveram imunidade.

\
**Pergunta:** os dados sustentam a alegação de 95%?

Aqui a preocupação é que o lote esteja pior do que o anunciado, então como definir as hipóteses nulas ($H_0$) e alternativa ($H_1$)?
...
**p-valor:** evidência contra $H_0$. Calculamos a probabilidade, sob $H_0$, de ocorrer um valor igual ou mais extremo ao valor observado no experimento.  
...
**Resultado do experimento:**
 ...
 p-valor: $P(T\geq `r acertos`)=`r valorp`$, onde $T\overset{H_0}{\sim}\mbox{Bin}(20, 1/2)$.
...
**p-valor** = $P(Z \geq `r zobs`) = `r round(valorpn,6)`$
...
arrows(3.5, yMax / 2,
       ...
       col = COL[1],
       lwd = 1.5)
text(3.5, yMax / 2, labels= paste0("p-valor = ", round(valorpn,6)),
     pos = 3,
     cex = 1,
     col = COL[1])
```
