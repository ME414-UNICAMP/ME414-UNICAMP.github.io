...
**Exemplo:** Voltando no exemplo das quantidades de sódio (mg) em 20 cereais matinais:
...
A população está representado em 1000 habitantes:
...
par(mar=c(2, 5, 2, 1), mgp = c(3.5, 0.6, 0))
boxplot(estados, main="População (em 1000 habitantes)", 
        ylab="População", type=2, col="lightblue", las = 1)
```
</center>
</div>
...
Vamos voltar no exemplo da quantidade de sódio (mg) em 20 cereais matinais
...
## Exemplo: Pesos de alunas de Educação Física  {.smaller .build}

Temos os pesos (em libras) de 64 alunas de Educação Física:
...
des <- data_frame("País"=c("Bélgica", "Dinamarca", "Alemanha", "Grécia",
                         "Espanha", "França", "Portugal", "Holanda",
                         "Luxemburgo", "Irlanda", "Itália", "Finlândia",
                         "Áustria", "Suécia", "Reino Unido"),
                  Taxa=desemprego)
des %>% kable() %>% kable_styling(bootstrap_options = c("striped"), full_width = FALSE, font_size = 22, position = "left")
```
</div>
...
Encontre os valores de $Q_1$, $Q_3$ e da mediana?
...
<div class="column-right">
<center>
```{r, echo=FALSE, fig.height=4.7, fig.width=3.5}
...
<center><img src="https://imgs.xkcd.com/comics/boyfriend.png" width=700></center>
<center>
<https://xkcd.com/539/>
</center>