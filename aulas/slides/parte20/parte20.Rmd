...
$H_{0}: \mu_{1}-\mu_{2}=\Delta_{0} \hspace{0.3cm} vs \hspace{0.3cm} H_A: \mu_{1}-\mu_{2} > \Delta_{0}$.

> Uma amostra aleatória de tamanho $n$ é coletada da população $X$ e calcula-se a média amostral, $\bar{x}$. Similarmente, para a população $Y$, temos $\bar{y}$ obtida a partir de uma amostra aleatória de tamanho $m$.
...
Se temos interesse em:
$H_{0}: \mu_1 - \mu_2 = \Delta_0 \qquad \mbox{vs} \qquad H_A: \mu_1 - \mu_2 \neq \Delta_0$.
...
$$ \frac{\bar X - \bar Y - (\mu_1 - \mu_2)}{\sqrt{S_p^2 \left( \frac{1}{n} + \frac{1}{m} \right)}} \sim t_{n+m-2}$$
...
Iguais e conhecidas ($\sigma_1^2 = \sigma_2^2 =\sigma^2$)    | $$Z= \frac{(\bar{X} - \bar{Y}) - \Delta_0}{ \sqrt{\sigma^2 \left( \frac{1}{n} + \frac{1}{m} \right)}} \sim N(0, 1)$$          | $$\mbox{rejeitar se }\mid z_{obs}\mid\geq z_{\alpha/2}$$       | $$2 P(Z\geq \mid z_{obs}\mid)$$
Iguais e desconhecidas ($\sigma_1^2 = \sigma_2^2 =\sigma^2$) | $$T\sim\frac{(\bar{X} - \bar{Y}) -\Delta_0}{ \sqrt{S_p^2 \left( \frac{1}{n} + \frac{1}{m} \right)}}\sim t_{n+m-2}$$                           | $$\mbox{rejeitar se }\mid t_{obs}\mid\geq t_{n+m-2,\alpha/2}$$ | $$2 P(T\geq \mid t_{obs}\mid)$$
...
Iguais e desconhecidas ($\sigma_1^2 = \sigma_2^2 =\sigma^2$) | $$T\sim\frac{(\bar{X} - \bar{Y}) -\Delta_0}{ \sqrt{S_p^2 \left( \frac{1}{n} + \frac{1}{m} \right)}}\sim t_{n+m-2}$$ | rejeitar se $t_{obs}\leq -t_{n+m+2,\alpha}$ | $P(T\leq t_{obs})$
...
Iguais e desconhecidas ($\sigma_1^2 = \sigma_2^2 =\sigma^2$) | $$T\sim\frac{(\bar{X} - \bar{Y}) -\Delta_0}{ \sqrt{S_p^2 \left( \frac{1}{n} + \frac{1}{m} \right)}}\sim t_{n+m-2}$$ | rejeitar se $t_{obs}\geq t_{n+m+2,\alpha}$ | $P(T\geq t_{obs})$
...
$H{}: \mu_{1}-\mu_{2} = 3 \qquad \mbox{vs} \qquad H_A: \mu_{1}-\mu_{2} \neq 3$
...
No entanto, $p$ é desconhecido. Iremos utilizar como estimativa para $p$, $\widehat p$, definido como o número de sucessos entre todos os elementos amostrados. 

Ou seja, $\widehat p$ é a proporção de sucessos na amostra toda, sem levar em consideração as populações, pois, sob $H_0$, $p_1=p_2$ (não há diferença entre as proporções das duas populações).
...
$H_0: p_1=p_2 \quad \mbox{vs} \quad H_A: p_1 < p_2$, que é equivalente a testar:  <br /> $H_0: p_1 - p_2 = 0 \quad \mbox{vs} \quad H_A: p_1 - p_2 < 0$.
