...
$$Z = \frac{(\bar X - \bar Y) - \Delta_0}{\sqrt{\frac{\sigma_1^2}{n} + \frac{\sigma_2^2}{m}}}  = \frac{(`r xbar` - `r ybar`) - `r Delta0`}{\sqrt{\frac{`r sigma21`}{`r n`} + \frac{`r sigma22`}{`r m`}}} =  `r abs(zobs)` = z_{obs}$$
...
Valor crítico para $\alpha=0.01$: $z_{0.005} = `r zc`$, ou seja, rejeita-se $H_0$ se $|z_{obs}|\geq `r zc`$.
...
Cálculo da estatística do teste:
$$Z = \frac{(\bar X_A - \bar X_B) - \Delta_0}{\sqrt{\sigma^2 \left(\frac{1}{n} + \frac{1}{m} \right)}} = \frac{(`r xbar` - `r ybar`) - `r Delta0`}{\sqrt{`r sigma2` \left(\frac{1}{`r n`} + \frac{1}{`r m`} \right)}} =  `r zobs` = z_{obs}$$
...
Nesse caso, a estatística do teste, sob $H_0$, é dada por:
$$T = \frac{\bar X_H - \bar X_M}{\sqrt{S_p^2 (\frac{1}{n} + \frac{1}{m})}} \sim t_{n+m-2}$$

Então,
$$t_{obs} = \frac{\bar x_H - \bar x_M}{\sqrt{s_p^2 \left(\frac{1}{n} + \frac{1}{m} \right)}}  = \frac{`r xbar`-`r ybar`}{\sqrt{`r s2p` \left( \frac{1}{`r n`} + \frac{1}{`r m`} \right)}} = `r zobs`$$

Para um nível de significância $\alpha = `r alpha`$, rejeitamos $H_0$ se $|t_{obs}| \geq t_{n+m-2,`r alpha/2`}$.

No caso, $|t_{obs}| = `r abs(zobs)` > `r zc` = t_{`r n + m -2`,`r alpha/2`}$. Portanto, temos evidências para rejeitar a hipótese de que as médias dos tempos de adaptação são iguais.
...
$$\widehat p = \frac{n_1 \widehat p_1 + n_2 \widehat p_2}{n_1 + n2}.$$
