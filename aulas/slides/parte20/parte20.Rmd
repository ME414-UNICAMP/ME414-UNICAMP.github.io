...
O p-valor é calculado de acordo com a hipótese alternativa.
...
$$\begin{aligned}
\mbox{p-valor} &= P(|Z| \geq |z_{obs}|) \\
&= 2P(Z \leq -|z_{obs}|)
\end{aligned}$$

<br />
**Conclusão**: Rejeita-se $H_0$ se p-valor $\leq \alpha$ ou, de forma equivalente, se $z_{obs}$ cai na região crítica (área cinza do gráfico).
...
Usando o valor observado da estatística do teste:
$$\mbox{p-valor} = P(Z \leq z_{obs})$$

<br />
**Conclusão**: Rejeita-se $H_0$ se p-valor $\leq \alpha$ ou, de forma equivalente, se $z_{obs}$ cai na região crítica (área cinza à esquerda do gráfico).
...
<br />
**Conclusão**: Rejeita-se $H_0$ se p-valor $\leq \alpha$ ou, de forma equivalente, se $z_{obs}$ cai na região crítica (área cinza à direita do gráfico).
...
<br />
Como é um teste bilateral, o p-valor é dado por:
$$\mbox{p-valor} = P(|Z| \geq |z_{obs}|) = P(|Z| \geq `r abs(zobs)`) = 2 P(Z \leq - `r abs(zobs)`) =  `r valorp`$$ 
...
**Fonte:** Adaptado das Notas de aula, Profa. Nancy Garcia.
...
Se a hipótese nula é verdadeira, temos que $\Delta_0 = \mu_A - \mu_B = 0$ e $Z \sim N(0, 1).$   
Note que a hipótese alternativa é do tipo $\neq$, então o teste é bilateral.


## Exemplo: Tecidos {.build}

Cálculo da estatística do teste:
$$z_{obs} = \frac{(\bar x_A - \bar x_B) - \Delta_0}{\sqrt{\sigma^2 \left(\frac{1}{n} + \frac{1}{m} \right)}} = \frac{(`r xbar` - `r ybar`) - `r Delta0`}{\sqrt{`r sigma2` \left(\frac{1}{`r n`} + \frac{1}{`r m`} \right)}} =  `r zobs`$$
...
Para um nível de significância $\alpha = `r alpha`$, rejeitamos $H_0$  se $|t_{obs}| \geq t_{n+m-2,`r alpha/2`}$. No caso, $|t_{obs}| = `r abs(zobs2)` < `r zc2` = t_{`r n + m -2`,`r alpha/2`}$. Portanto, não temos evidências para rejeitar a hipótese de que as médias dos dois tecidos são iguais.
...
O p-valor e a conclusão se dá de acordo com a hipótese alternativa na tabela:
</font>
...
Alunos de uma turma de ME414 foram recrutados para um estudo e cada um recebeu a seguinte informação através do Google Forms:
...
p-valor = $P(Z \leq z_{obs}) = P(Z \leq `r zobs`) = `r valorp` > \alpha$. Portanto, não rejeitamos $H_0$.
